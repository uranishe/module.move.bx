...
final class BlueprintImporter
{
    ...
    {
        $dictionaries = $this->blueprint['dictionaries'] ?? [];
        ...
        foreach (self::DICTIONARY_MAP as $code => $dictionary)
        {
            $items = $dictionaries[$code]['items'] ?? null;
            if (!is_array($items))
            {
                continue;
            }
            ...
            try
            {
                $this->importDictionaryItems($code, $dictionary['entity_id'], $items);
            }
            catch (\Throwable $e)
            {
                $this->errors[] = 'Ошибка импорта справочника ' . $dictionary['title'] . ': ' . $e->getMessage();
            }
        }
    }

    private function importDictionaryItems(string $dictionaryCode, string $entityId, array $items): void
    {
        ...
        foreach ($items as $item)
        {
            ...
            if ($statusId === '')
            {
                $this->report['dictionaries'][$dictionaryCode]['skipped'][] = [
                    'reason' => 'Пустой STATUS_ID',
                ];
                continue;
            }

            $payload = $this->sanitizeStatusRow($item, $entityId, $statusId, 'lead');
            $action = isset($existing[$statusId]) ? 'update' : 'create';
